...
def run(wt2: str, position_name: str, position_key: str, page: int, log_level) -> str:
    global _cookie
    global _page
    global _position_match
    global _school_list
    _log_level = log_level  # 日志等级，默认INFO，可选DEBUG，SUCCESS，ERROR，WARNING等
    logger.remove()
    logger.add(sys.stderr, level=_log_level)
    _wt2 = wt2
    _cookie = f"wt2={_wt2}"
    _encryptJobId = query_position(position_name)  # 要匹配的职位，精确匹配
    _position_match = position_key  # 要匹配的职位
    _page = page  # 查询的页数，每页15条应聘者数据，网页上一次最多只给查30页的数据，不支持30页以上的参数
    ...
    try:
        ...
        return "没有找到schoolList.txt文件，请新建该文件并写入院校名单信息"
